...
rpcc::caller::caller(unsigned int xxid, unmarshall *xun): xid(xxid), un(xun), done(false)
{
...
rpcc::call1(unsigned int proc, marshall &req, unmarshall &rep, TO to)
{
	...
	jsl_log(JSL_DBG_2, "rpcc::call1 %u call done for req proc %x xid %u %s:%d done? %d ret %d \n",
			clt_nonce_, proc, ca.xid, inet_ntoa(dst_.sin_addr), ntohs(dst_.sin_port), ca.done, ca.intret);
...
/**
 PollMgr's thread is being used to make this upcall from connection object to rpcc.
...
rpcs::dispatch(djob_t *j)
{
	...
	jsl_log(JSL_DBG_2, "rpcs::dispatch: rpc %u (proc %x, last_rep %u) from clt %u for srv instance %u \n",
	...
	if(h.srv_nonce != 0 && h.srv_nonce != nonce_){
		jsl_log(JSL_DBG_2, "rpcs::dispatch: rpc for an old server instance %u (current %u) proc %x\n",
	...
	if(h.clt_nonce){
		// have i seen this client before?
		{
			...
			if(reply_window_.find(h.clt_nonce) == reply_window_.end()){
				...
				jsl_log(JSL_DBG_2, "rpcs::dispatch: new client %u xid %d chan %d, total clients %d\n",
	...
	switch (stat){
		case NEW: // new request
			...
			jsl_log(JSL_DBG_2, "rpcs::dispatch: sending and saving reply of size %d for rpc %u, proc %x ret %d, clt %u\n",
...
/**
   1. rpcs::dispatch:
   1) calls this when an RPC request arrives.
   2) checks to see if an RPC with xid from clt_nonce has already been received. If not:
      -- remembers the request in reply_window_.
...
**/
rpcs::rpcstate_t
rpcs::checkduplicate_and_update(unsigned int clt_nonce, unsigned int xid, unsigned int xid_rep, char **b, int *sz)
{
	ScopedLock rwl(&reply_window_m_);
...
/**
   1. rpcs::dispatch calls add_reply when it is sending a reply to an RPC and passes the return value in b and sz.
   2. add_reply() should remember b and sz.
   3. free_reply_window() and checkduplicate_and_update is responsible for calling free(b).
**/
void
