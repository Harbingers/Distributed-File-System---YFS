...
/**
 ...
 -- they must be methods of some class to simplify rpcs::reg().
 -- a server process can have handlers from multiple classes.
**/
class srv {
	public:
...
/**
 ...
 -- the caller also gets to see the int return value as the return value from rpcc::call().
 -- rpcs::reg() decides how to unmarshall by looking at these argument types,
    so this function definition does what a .x file does in SunRPC.
 **/
int
