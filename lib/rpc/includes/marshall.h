...
enum {
	...
	DEFAULT_RPC_SIZE = 1024,
#if RPC_CHECKSUMMING
	//size of rpc_header includes a 4-byte int to be filled by tcpchan and uint64_t checksum
	RPC_HEADER_SIZE = static_max<sizeof(req_header), sizeof(reply_header)>::value + sizeof(rpc_size_t) + sizeof(rpc_checksum_t)
#else
    RPC_HEADER_SIZE = static_max<sizeof(req_header), sizeof(reply_header)>::value + sizeof(rpc_size_t)
#endif
};
...
class marshall {
	private:
		...
		int _capacity;  // Capacity of the buffer
		int _ind;       // Read/write head position

	public:
		marshall() {
			_buf = (char *) malloc(sizeof(char)*DEFAULT_RPC_SIZE);
			VERIFY(_buf);
			_capacity = DEFAULT_RPC_SIZE;
		...
		**/
		std::string get_content() {
			return std::string(_buf+RPC_HEADER_SIZE, _ind-RPC_HEADER_SIZE);
		}
...
class unmarshall {
	...
	public:
		unmarshall(): _buf(NULL), _size(0), _ind(0), _ok(false) {}
		unmarshall(char *b, int size): _buf(b), _size(size), _ind(), _ok(true) {}
		/**
		 take the content which does not exclude a RPC header from a string
		**/
		unmarshall(const std::string &s) : _buf(NULL), _size(0), _ind(0), _ok(false) {
			take_content(s);
		}
		~unmarshall() {
			if (_buf) free(_buf);
